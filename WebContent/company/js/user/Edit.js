define(['dialogPlus', 'validate', 'validateExtend'],function (dialog) {
    return Class.extend({
        init: function () {
                ...
                $("#add-user").find("input").each(function () {
                    if ($(this).attr("type") == "radio" && $(this).attr("checked")) {
                        ...
                        data.push($(this).attr("name") + ":" + value);
                         if ($(this).attr("name") == "credits" && parseInt(value) > data_json.shareCredits) {
                            _this.myTips({ content: "上级可用餘額：" + data_json.shareCredits, obj: $(this), myclick: true });
                            data = false;
                            return false;
                            ...
                            _this.myTips({ content: "上級最高可設占成：" + data_json.stintOccupyMax + "%", obj: $(this), myclick: true });
                            data = false;
                            return false;
        ...
        onPageLoad: function () {
            ...
            _this.superChange();
            //上级总监下拉选择
            $(_this.formSelector).on("change","select[name='shareName']",function () {
                _this.superChange();
            });
