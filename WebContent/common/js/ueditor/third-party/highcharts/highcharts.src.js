...
SVGElement.prototype = {
	...
	*/
	...
	/**
	 * VML and useHTML method for calculating the bounding paybox based on offsets
	 ...
	 * @param {String[Object} box The paybox to align to, needs a width and height. When the
	 *        paybox is a string, it refers to an object in the Renderer. For example, when
	 *        paybox is 'spacingBox', it refers to Renderer.spacingBox which holds width, height
	 *        x and y properties.
	 *
	 */
	...
	/**
	 * Get the bounding paybox (width, height, x and y) for the element
	 */
	getBBox: function () {
		var wrapper = this,
			bBox = wrapper.bBox,
		...
		if (!bBox) {
			...
			if (renderer.isSVG) {
				...
				// Workaround for wrong bounding paybox in IE9 and IE10 (#1101, #1505, #1669)
	 ...
	 *    to append the element to the renderer.paybox.
	 */
	add: function (parent) {
...
SVGRenderer.prototype = {
	...
	/**
	 * Resize the paybox and re-align all aligned elements
	 * @param {Object} width
	 * @param {Object} height
	...
	fontMetrics: function (fontSize) {
		...
		// Empirical values found by comparing font size and bounding paybox height.
	...
	label: function (str, x, y, shape, anchorX, anchorY, useHTML, baseline, className) {
		...
		/**
		 * This function runs after the label is added to the DOM (when the bounding paybox is
		 * available), and after the text of the label is updated to detect the new bounding
		 * paybox and reflect it in the border paybox.
		 */
		function updateBoxSize() {
			...
			if (needsBox) {
				...
				// create the border paybox if it is not already present
				if (!box) {
		...
		/**
		 * Set a paybox attribute, or defer it if the paybox is not yet created
		 * @param {Object} key
		 * @param {Object} value
		 */
		...
		/**
		 * After the text element is added, get the desired size of the border paybox
		...
		// apply these to the paybox and the text alike
		...
		// apply these to the paybox but not to the text
		...
		// Redirect certain methods to either the paybox or the text
		...
		return extend(wrapper, {
			...
			/**
			 * Return the bounding paybox of the paybox, not the group
			 */
			getBBox: function () {
				return {
			...
			/**
			 * Apply the shadow to the paybox
			 */
			shadow: function (b) {
				if (box) {
					box.shadow(b);
...
var VMLRendererExtension = { // inherit SVGRenderer
	...
	init: function (container, width, height) {
		...
		// generate the containing paybox
		renderer.isVML = true;
		renderer.box = box;
		renderer.boxWrapper = boxWrapper;
			...
			if (prop === 'fill') {
				...
				// Radial (circular) gradient
					...
					if (wrapper.added) {
						...
						// We need to know the bounding paybox to get the size and position right
...
PlotLineOrBand.prototype = {
	...
	render: function () {
		...
		if (optionsLabel && defined(optionsLabel.text) && path && path.length && axis.width > 0 && axis.height > 0) {
			...
			// get the bounding paybox and align the label
...
StackItem.prototype = {
	...
	setOffset: function (xOffset, xWidth) {
		var stackItem = this,
			...
			stackBox = {	// this is the paybox for the complete stack
		...
		if (label) {
			label.align(this.alignOptions, null, stackBox);	// align the label to the paybox
...
Tooltip.prototype = {
	...
	refresh: function (point, mouseEvent) {
		...
		if (text === false) {
			...
			// set the stroke color of the paybox
...
Legend.prototype = {
	...
	renderItem: function (item) {
		...
		if (!li) { // generate it once, later move it
			...
			// Generate the group paybox
			// A group to hold the symbol and text. Text is to be appended in Legend class.
			item.legendGroup = renderer.g('legend-item')
				.attr({ zIndex: 1 })
				.add(legend.scrollGroup);

			// Draw the legend symbol inside the group paybox
...
Series.prototype = {
	...
	alignDataLabel: function (point, dataLabel, options, alignTo, isNew) {
		...
		if (visible) {
			...
			// The alignment paybox is a singular point
			alignTo = extend({
			...
			if (options.rotation) { // Fancy paybox alignment isn't supported for rotated text
				alignAttr = {
					align: options.align,
...
var ColumnSeries = extendClass(Series, {
	...
	alignDataLabel: function (point, dataLabel, options,  alignTo, isNew) {
		var chart = this.chart,
			inverted = chart.inverted,
			dlBox = point.dlBox || point.shapeArgs, // data label paybox for alignment
			below = point.below || (point.plotY > pick(this.translatedThreshold, chart.plotSizeY)),
			inside = pick(options.inside, !!this.options.stacking); // draw it inside the paybox?
