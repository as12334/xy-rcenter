define(['jquery'], function (jquery) {
    ...
    return Class.extend({
        ...
        bindEvent : function(_this) {
            ...
            $(document).on("click", ".is_auto_complete", function (e) {
                e.stopPropagation();
            });
                    ...
                    $.each(_relSelect.split(","), function (index, item) {
                        ...
                        $("#mainFrame").children("div").each(function () {
                            if($(this).css("display")=="block"){
                                _relobj=$(this).find("[selectDiv='" + item + "']");
                            }
                        });
            ...
            $(document).on("input propertychange", ".select_input", function (e) {
                _this.autoComplete(e);
            })
        },
            ...
            if(relSelectPath) {
                ...
                if(matchs) {
                    $.each(matchs, function (index, item) {
                        ...
                        $("#mainFrame").children("div").each(function () {
                            if($(this).css("display")=="block") {
                                _relobj = $(this).find("[selectDiv='" + item.replace(/#/g, "") + "'] input");
                            }
                        });
                        var _relObj = $(document).find("[selectDiv='" + item.replace(/#/g, "") + "'] input");
                        var val;
                        if(typeof _relobj == "undefined"){
                            val = _relObj.attr("value");
                        }else{
                            val = _relobj.attr("value");
                        }
                        if (val && val != "") {
        ...
        autoComplete: function (e) {
            //1.文本框没有输入内容恢复原有下拉选项
            //2.匹配下拉选项的数据，如果没有匹配到就隐藏,匹配到就显示
            var target = $(e.currentTarget);
            var inputVal = target.val();
            var parent = target.parent();
            var lis = parent.nextAll("li");
