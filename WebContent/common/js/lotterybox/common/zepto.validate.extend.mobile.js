...
(function( factory ) {
    ...
    $.validator.setDefaults({
        ...
        keypressDelay: 10, //扩展的属性：延迟验证毫秒数
    ...
    $.validator.addMethod("integerPrecision", function(value, element, param) {
        ...
        return match;
    }, window.top.message["integerPrecision"]);
    ...
    $.validator.addMethod("fractionPrecision", function(value, element, param) {
        ...
        return match;
    }, window.top.message["fractionPrecision"]);
    ...
    /**
     * 显示隐藏提示,如果有提示信息显示,否则隐藏.
     * @param elem
     * @param messsage
     */
    function showHideTip ( elem , error_message  ){
        console.log(error_message);
        var $elem_parent = $(elem).parent();
        var $error_tag = $elem_parent.find("p.gb-input-help");
        if (error_message){
            /*show*/
            if($error_tag.length){
                $error_tag.show().find("span.error-message").text(error_message)
            } else {
                var errorElement = getErrorElement(error_message);
                $(errorElement).appendTo($elem_parent)
            }
        } else {
            /*hide*/
            $elem_parent.find("p.gb-input-help").hide();
        }
    }

    function getErrorElement(message){
        return  '<p class="gb-input-help"><span class="mui-icon mui-icon-info-filled"></span><span class="error-message">' + message + '</span></p>'
    }
...
(function(){
    var $form = $("form");
    if ( $form.length === 1 ){
        var rule = eval("({" + $("#validateRule",$form).text() + "})");
        $form.validate(rule);
    }
})();