    ...
    return Class.extend({
        ...
        initDatePick: function ($ojb) {
            ...
            $.each($ojb, function (index, item) {
                ...
                if (data) {
                    var pageSelect=window.top.topPage.cuid?"."+window.top.topPage.cuid:"";
                    if (data.useRange == true) {
                        ...
                        optionRange.startDateElement=$(pageSelect + " input[name='" + data.startName + "']").length>0?$(pageSelect + " input[name='" + data.startName + "']"):$("input[name='" + data.startName + "']");
                        optionRange.endDateElement=$(pageSelect + " input[name='" + data.startName + "']").length>0?$(pageSelect + " input[name='" + data.endName + "']"):$("input[name='" + data.endName + "']");
                    } else {
                        if (data.startDate) {
                    ...
                    //页面回退时回填数据
                    if (data.useRange) {
                        var startField = $(pageSelect + " input[name='" + data.startName + "']").length>0?$(pageSelect + " input[name='" + data.startName + "']"):$("input[name='" + data.startName + "']");
                        var endField = $(pageSelect + " input[name='" + data.startName + "']").length>0?$(pageSelect + " input[name='" + data.endName + "']"):$("input[name='" + data.endName + "']");
                        ...
                        if(data.startName==$text.attr("name")) {
                            ...
                            optionStart.startDateElement=$(pageSelect + " input[name='" + data.startName + "']").length>0?$(pageSelect + " input[name='" + data.startName + "']"):$("input[name='" + data.startName + "']");
                            optionStart.endDateElement=$(pageSelect + " input[name='" + data.startName + "']").length>0?$(pageSelect + " input[name='" + data.endName + "']"):$("input[name='" + data.endName + "']");
                            optionStart.startDate=function(){
                                var startDate=$(pageSelect + " input[name='" + data.startName + "']").length>0?$(pageSelect + " input[name='" + data.startName + "']"):$("input[name='" + data.startName + "']");
                                if(startDate.val()){
                        ...
                        if(data.endName==$text.attr("name")) {
                            ...
                            optionEnd.startDateElement=$(pageSelect + " input[name='" + data.startName + "']").length>0?$(pageSelect + " input[name='" + data.startName + "']"):$("input[name='" + data.startName + "']");
                            optionEnd.endDateElement=$(pageSelect + " input[name='" + data.startName + "']").length>0?$(pageSelect + " input[name='" + data.endName + "']"):$("input[name='" + data.endName + "']");
        ...
        onPageLoad: function () {
            ...
            /*var fd = $(this.formSelector + " textarea,input:not('.daterangepickers input'):not('[readonly]'):not('[type=radio]'):input:visible:enabled:first")[0];
            if (fd && fd.name !== 'paging.pageNumberText')
            {
                var value = $(fd).val();
                $(fd).focus().val(value);
            }*/
        ...
        SmoothlyMenu: function () {
            if (!$('body').hasClass('mini-navbar') || $('body').hasClass('body-small')) {
                ...
                $('#side-menu').hide();
                //展开时添加滚动条
                $('#side-menu').parent('div').css({'overflow-y': 'scroll'});
                ...
                $('#side-menu').parent('div').css({'overflow-y': ''});
         ...
         * @param callback 回调
         */
        baseInitSwitch: function ($switch, text, checkMethod, callback) {
            var _this = this;
            _this.unInitSwitch($switch)
                ...
                .on('switchChange.bootstrapSwitch', function (event, status) {
                    var element = $(this);
                    var msg = "";
                    element.bootstrapSwitch('indeterminate', true);
                    if (status) {
                        if (text && text.onText){
                            msg = text.onText;
                        }else {
                    ...
                    window.top.topPage.showConfirmMessage(msg, function (bol) {
                        if (bol) {
                            if (checkMethod){
                                event.page = _this;
                                var returnVal = window.top.topPage.doPageFunction(event, checkMethod, {});
                                if (returnVal && returnVal == true){
                                    _this.changeBootstrapSwitch(element, status, true);
                                    if (callback){
                                        window.top.topPage.doPageFunction(event, callback, {});
                                    }
                                }else {
                                    _this.changeBootstrapSwitch(element, status, false);
                                }
                            }else {
                                _this.changeBootstrapSwitch(element, status, true);
                            }
        ...
        changeBootstrapSwitch: function (element, status, change) {
            if (change && change == true) {
                element.bootstrapSwitch('state', status, true);
            } else {
                element.bootstrapSwitch('indeterminate', false);
        ...
        /**
         * 为了能将单选、多选框存入缓存，解决多标签页切换的bug，要做追加操作
         * @param selector 选择器
         */
        overwriteInputEvent: function (selector) {
            var _this = this;
            if (_this.notEmpty(selector)){
               $(selector).on('change', ':radio, :checkbox', function (e, opt) {
                   $(this).attr('checked', "true");//以字符串的方式兼容
                   $('input[name="' + $(this).attr('name') + '"]', selector).each(function (i, e) {
                       if (!$(this).is(':checked')) {
        ...
        notEmpty: function () {
            for (var i = 0; i < arguments.length; i++) {
                if (arguments[i] == null || arguments[i] == '' || arguments[i] == undefined) return false;
            }
            return true;
        },
        /**
        ...
        /**
         * 将所有input的name和value组成键值对
         *
         * @param selector
         * @returns {{}}
         */
        getAjaxJsonData: function (selector) {
            var _this = this;
            var data = {};
            if (_this.notEmpty(selector)) {
         ...
         * @param data 数据
         */
        asyncFillData: function (selector, htmlType, keys, data) {
            var _this = this;
            if (_this.notEmpty(selector, keys, data) && $(selector).length == 1) {
                for (var i = 0; i < keys.length; i++) {
                    var node = $('[data-param="' + keys[i] + '"]', selector);
                    if (htmlType == 'span') {
                        node.text(data[keys[i]]);
                    } else if (htmlType == 'input') {

                    }else if(htmlType == 'html') {
                        node.html(data[keys[i]]);
                    }
                }
            }
        }
