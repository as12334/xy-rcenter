...
<%@ attribute name="isAutoComplete" type="java.lang.String" required="false" description="是否需要显示文本框并自动补全过滤" %>
...
<ul class="dropdown-menu" role="menu" aria-labelledby="dropdownMenu1" style="${cssStyle}">
    ...
    <c:if test="${isAutoComplete}">
        <div class="input-group label-search is_auto_complete">
            <input type="text" class="form-control select_input">
        </div>
    </c:if>
